...
exports.fileprint = function (req, res) {
  ...
  jsonfile.readFile(filepath, function (err, obj) {
    var userid = obj.userid;
    connection.query('SELECT * FROM prophesierusers WHERE userid = ?', [userid], function (error, results, fields) {
            if (error) {
              ...
              if (results.length > 0) {
                ...
                connection.query('UPDATE prophesierusers SET printCount = ? WHERE userid = ?', [printCount, userid], function (error, results, fields) {
                  if (error) {
